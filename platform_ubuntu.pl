...
make_platform_Tartarus_IITG(Ip,Port) :-                                         %% create platform at +port +Ip
   (platform_state(on)->                                                        %% To verify
	...
	ansi_format([bold,fg(green)],'                   (Ver.2020.1, 27th April 2020~w',[')']),nl,nl,
...
espeak_present:-
		...
		exists_file("/usr/bin/espeak")-> 
		asserta(subt(0)),
		asserta(tt(1));
		asserta(subt(0)),
		asserta(tt(0)).

start_tartarus(Ip,Port,Token):-															%by Suraj Kumar Pandey
		check_syntax(start_tartarus,_Ip,Port,Token),       
		start_db,
                %writeln('In Start Tar'),
...
agent_post(platform,(Ip_receiver,Port_receiver),FunctionList):-                         %% +Receiver_Agent +Ip_receiver +Port_receiver +Sender_agent +Function
        %repeat,   
        (compound(Ip_receiver), term_to_atom(Ip_receiver, IP_Send) ; IP_Send = Ip_receiver),      %added by Divya for tarpy feature                                                                                                                                                       %% Keep sending message until -Ack is received from other side
        catch(send_message(IP_Send,Port_receiver,FunctionList,Ack),Error,(writeln(Error),mutex_unlock(send_message_mutex),fail)),   %% if present then send message
...
agent_post(_Platform,(_Ip_receiver,_Port_receiver),_FunctionList):-
		...
		writeln('Enter a valid IP address and Port number.'),fail,!.  %added by Menaxi J Bagchi
...
create_mobile_agent(GUID,Handler,List):- check_syntax(create_mobile_agent,_GUID,Handler,List),agent_create(GUID,Handler),  %added by Menaxi J Bagchi% 
										 check_handler(Handler,3),                                                         %added by Menaxi J Bagchi%  
                                         add_token(GUID,List).
...
agent_clone(GUID1,(Send_Ip,Send_Port),GUID2):-
        ...
        agent_post(platform,(Send_Ip,Send_Port),[handler,platform,(Send_Ip,Send_Port),send(GUID2,Handler,PayloadList,CloneCode,ListOfTokens2,TT)]),!.
...
/*get_destination_Ip(Dest_ip):-
         retractall(get_destination_Ip(_)),
         assert(get_destination_Ip(Dest_ip)).
         %writeln(Dest_ip).

get_destination_port(Dest_port):-
        retractall(get_destination_port(_)),
        assert(get_destination_port(Dest_port)).*/
...
/*handler(platform,(Ip,Port),send_ackm(GUID,_X)):-                                                                
		...
		L = (GUID,Dest_ip,_,_), L3 = [(GUID,Dest_ip,_,_)], get_hoptime(L1),
...
handler(platform,(Ip,Port),send_ackm(GUID,_X)):- 
       ...
       (Il2=:=1,                                                            
        ...
        post_agent(platform,(Dest_ip,Dest_port),[predicate_tar,GUID,Source_ip,Source_port,Dest_ip,Dest_port,Ya2,Start_time,Time_taken]),
        post_agent(platform,(Dest_ip,Dest_port),[predicate_tar1,GUID,Time_taken]),
...
handler(platform,(_Ip,_Port),send_nackm(_GUID)):- print_message(error,'handler(platform,(Ip,Port),send_nackm(GUID)):-'),!.


/*handler(platform,(_Ip,_Port),send_nackm(GUID,_Handler,_PayloadList,_ListOfTokens,_AgentCode)):-
        %ttyflush,
        writeln('--------negative ACK received-----------'),
		writeln('token did not match'),
        nl,
        write('Agent '),write(GUID),write(' was not allowed to enter the destination.'),
        nl,
		assert(agent_GUID(GUID,Handler,(Ip,Port))), writeln(Ip), writeln(Port),
		assert(agent_payload(GUID,PayloadList)),
        assert(agent_token(GUID,ListOfTokens)), writeln(Handler), writeln(ListOfTokens),
        assert_code_Tartarus_IITG(AgentCode),
        %agent_execute(GUID,(Ip,Port),Handler),
		(not(isexist_agent(GUID,(Ip,Port),Handler))->writeln('false');writeln(true),writeln(GUID),writeln(Ip),writeln(Port),writeln(Handler)).
		platform_Ip(IP),
		((not(isexist_agent(GUID,(IP,Port),Handler)))-> create_mobile_agent(GUID,(IP,Port),Handler,ListOfTokens),writeln(GUID),writeln(IP),writeln(Port),writeln(Handler),writeln(ListOfTokens);writeln('error occured'),writeln(GUID),writeln(IP),writeln(Port),writeln(Handler)),
        %catch((message_queue_create(queue1)->writeln('successful')),Error,(writeln('error occured'))),
...
handler( platform, (Ip_sender,Port_sender), send(GUID,Handler,PayloadList,[Code|CodeList],ListOfTokens,_BackTime)):-                             
       ...
       (platform_token(PToken)->
		...
		(platform_token(PToken)->
                (member(PToken,ListOfTokens)->
                        (agent_post(platform,(Ip_sender,Port_sender),[handler,platform,(Ip_sender,Port_sender),send_ackm(GUID)])),
						writeln('successful');
                        (agent_post(platform,(Ip_sender,Port_sender),[handler,platform,(Ip_sender,Port_sender),send_nackm(GUID,Handler,PayloadList,ListOfTokens,AgentCode)])),
						writeln('unsuccessful'),fail
		...
		*/
		(agent_GUID(GUID,Handler,(_,_))->   % Tushar - added to kill agent already present with same name and handler name
				writeln('Agent already present'),
				agent_kill(GUID)
				;
				nothing
		),
        write('Agent bearing '),write(GUID),
        write(' has arrived at this platform. '),nl,
        get_platform_details(Platform_Ip,Platform_Port),
        (thread_peek_message(clean_agent_queue,agent_guid(GUID))->
                cleanup_wait(GUID)
                %handler(platform,(Platform_Ip,Platform_Port),clean_agent(GUID))
        ...
        get_time(Time),                                                 %Time is the time when the token is matched and the agent is created on the platform
		...
		*/
        ...
        assert(agent_GUID(GUID,Handler,(Platform_Ip,Platform_Port))),
        get_time(Time),                                                 %Time is the time when the token is matched and the agent is created on the platform
        (compound(Ip_sender), term_to_atom(Ip_sender, IP_Send) ; IP_Send = Ip_sender), %added by Divya for tarpy feature
        agent_post(platform,(IP_Send,Port_sender),[handler,platform,(IP_Send,Port_sender),send_ackm(GUID,Time)]),
        writeln('Now executing the agent'),
        writeln('agent name is ':GUID),
        agent_execute(GUID,(Platform_Ip, Platform_Port),Handler),
        nl,write('Agent has been created at this port : '),
        %write(Port2),nl,
        %nl,
...
% list_agent/0 prints all the agents and their ports onto the console. list_agent/2 lets you do the same, accept that
...
predicate_tar(GUID,Source_ip,Source_port,Dest_ip,Dest_port,Ya2,Start_time,Time_taken):-
...
predicate_tar1(GUID,Time_taken):- 
...
%added for tarpy by Divya
check_tarpy(X,Y):-
        writeln('In Tarpy'),
        mutex_create(tarpymutex),
        with_mutex(tarpymutex,(retractall(X), writeln('Retracted'), assert(Y), writeln('Asserted'))),
        mutex_destroy(tarpymutex).